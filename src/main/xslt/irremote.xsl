...
<xsl:stylesheet
    ...
    <xsl:param name="frequencyType" select="'unsigned int'"/>
    ...
    <xsl:param name="constAttribute" select="'const'"/>

    <xsl:param name="trailingGaps" select="false()"/>
    <!--xsl:param name="trailingGaps" select="true()"/-->
...
typedef uint16_t microsecondsType;
    ...
    <xsl:template match="NamedProtocol">
        ...
        <xsl:apply-templates select="Protocol/static-variables/*"/>
        <xsl:apply-templates select="Protocol/instance-variables/*"/>
        <xsl:text>
#if SEND_</xsl:text>

        <xsl:value-of select="upper-case(@name)"/>
        <xsl:text xml:space="preserve">&#10;</xsl:text>
        <xsl:apply-templates select="Protocol/function"/>
    ...
    <xsl:template match="BitSpec[../@bitDirection='lsb']">
        <xsl:value-of select="$returnType"/>
        <xsl:text> bitSpec(</xsl:text>
        <xsl:value-of select="$parameterType"/>
        <xsl:text> data, </xsl:text>
        <xsl:value-of select="$parameterType"/>

        <xsl:text xml:space="preserve"> width) {&#10;</xsl:text>
        ...
        <xsl:text>switch (data &amp; bitMask) {&#10;</xsl:text>
        <xsl:apply-templates select="BareIrStream" mode="function-def"/>
        <xsl:text>default:&#10;assert(false);&#10;break;&#10;</xsl:text>
        <xsl:text xml:space="preserve">}&#10;</xsl:text>
        <xsl:text xml:space="preserve">}&#10;</xsl:text>
        <xsl:text xml:space="preserve">}&#10;</xsl:text>
    ...
    <xsl:template match="instance-variable[@name='sumOfDurations' and $trailingGaps=false()]"/>
    ...
    <xsl:template match="funccall[@name='Flash' or @name='Gap' or @name='Extent']">
        <xsl:value-of select="@name"/>
        <xsl:text>(</xsl:text>
    ...
    <xsl:template match="funccall[@name='Extent' and $trailingGaps=false() and position()=last() and parent::body]
                        |funccall[@name='Gap' and $trailingGaps=false() and position()=last() and parent::body]"
    mode="send" priority="100">
        <xsl:text xml:space="preserver">Gap(0); // IRremote proper ignores trailing spaces&#10;</xsl:text>
    </xsl:template>
    ...
    <xsl:template match="assignment[@name='sumOfDurations' and $trailingGaps=false()]" mode="send"/>
    ...
    <!--xsl:template match="body" mode="send">
        <xsl:text>{
        </xsl:text>
        <xsl:text>enableIROut(</xsl:text>
